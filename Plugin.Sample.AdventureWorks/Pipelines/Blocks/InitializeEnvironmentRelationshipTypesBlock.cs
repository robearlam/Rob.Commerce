...
namespace Plugin.Sample.AdventureWorks
{
    ...
    /// <summary>
    /// Defines a block which bootstraps relationship types in the AdventureWorks sample environment.
    /// </summary>
    /// <seealso>
    ///     <cref>
    ...
    ///     </cref>
    /// </seealso>
    [PipelineDisplayName(AwConstants.Pipelines.Blocks.InitializeEnvironmentRelationshipTypesBlock)]
    public class InitializeEnvironmentRelationshipTypesBlock : PipelineBlock<string, string, CommercePipelineExecutionContext>
    {
        ...
        {
            var artifactSet = "Environment.AdventureWorks.RelationshipTypes-1.0";
        ...
        private async Task<string> CreateRelationship(string relationShipName, string description, string sourceType, string targetType, CommercePipelineExecutionContext context)
        {
            ...
            var findResult = await this._findEntityPipeline.Run(new FindEntityArgument(typeof(RelationshipDefinition), itemId, false), context.CommerceContext.GetPipelineContextOptions());
