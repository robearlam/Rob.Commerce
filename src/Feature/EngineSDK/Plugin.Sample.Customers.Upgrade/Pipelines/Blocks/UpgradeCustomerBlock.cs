...
namespace Plugin.Sample.Customers.Upgrade
{
    ...
    /// </cref>
    /// </seealso>
    [PipelineDisplayName(CustomersUpgradeConstants.Pipelines.Blocks.UpgradeCustomerBlock)]
    public class UpgradeCustomerBlock : PipelineBlock<Customer, Customer, CommercePipelineExecutionContext>
    {
        ...
        public override async Task<Customer> Run(Customer arg, CommercePipelineExecutionContext context)
        {
            ...
            await this._deleteEntityPipeline.Run(new DeleteEntityArgument($"{EntityIndex.IndexPrefix<Customer>("Id")}{arg.Email}"), context);
